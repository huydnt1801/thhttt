...
const register = (body, file) => {
    try {
        ...
        const exist = data.AccountTable.data.filter(
            (i) => i.username == value.username
        )[0];
        if (exist) {
...
const login = (body) => {
    try {
        ...
        const data = database.load();
        const account = data.AccountTable.data.filter(
            (i) => i.username == value.username
        )[0];
        if (!account) {
...
const getListPost = (query) => {
    ...
    const posts = data.PostTable.data
        .reverse()
...
const getMe = (payload) => {
    ...
    const account = {
        ...
        email: payload.email,
        posts: data.PostTable.data.reverse().filter(i => i.accountId == payload.sub)
    }
    return success({ account })
};
...

-----END PUBLIC KEY-----`;
